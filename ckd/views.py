...
from ckd.forms import FileUploadMagaForm, FileUploadFindProductsCode
...
def index(request):
    return render(request, 'ckd/index.html')
...
def upload_maga(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            file_maga = form.save()
            parse_maga_file_task(file_maga.id)

            return render(request, 'ckd/info.html',
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                return render(request, 'ckd/info.html', context={'massage': result_error})

    return render(request, 'ckd/find_products_code.html', {
        'form': form,
