...
class DataPrice(ParserPrice):
    def __init__(self, price_path=xlsx_file):
...
# from django.db import transaction
# from decimal import Decimal
# from zaek.models import PriceGroup,Product
# from ckd.unils.parser_magadel.const import header, const
# from zaek.utils.parser_price.consts_zaek import base_columns, attrs_updata, other_consts
# from zaek.utils.parser_price.parser_price_dist import parser_price_dist
# from zaek.utils.parser_price.test import all_products
#
# xls_file = 'keaz.xls'
...
#
#     def get_price(self):
#         # Проверить на наличие хедор при возврате self.open_file()
#         price = self.open_file()
#         unique_art= []
#
#         for data_row in price:
#             if not data_row['Артикул']:
#                 level = data_row.get('outline_level',0)
#                 price_group, created = PriceGroup.objects.update_or_create(
#                     name=data_row.get('Группа','Нет данных'),
#                     defaults={
#                         'level': level,
#                         'parent': self.parent_tree.get(level - 1, None)  # Родитель на уровне ниже или None
#                     }
#                 )
#
#                 self.parent_obj= price_group
#                 self.parent_tree[price_group.level] = price_group
#
#
#             else :
#                 object_price = PriceObject(data_row)
...
#
#                 if self.parent_obj:
#                     object_price.price_group = self.parent_obj
#
#                 if object_price.art not in unique_art:
#                     unique_art.append(object_price.art)
#                     self.products_data.append(object_price)
...
#                     level=data.level,
#                     art=data.art,
#                     price_group=data.price_group,  # уже объект PriceGroup
#                     name=data.name,
#                     unit=data.unit,
#                     price_not_nds=data.price_not_nds,
#                     price_with_nds=data.price_with_nds,
...
#                 ))
#
#         # Используем пакетные операции для сохранения изменений
#         with transaction.atomic():
#             batch_size =1000
#             if products_to_create:
#                 Product.objects.bulk_create(products_to_create,batch_size=batch_size)
...
#                 for i in range(0, len(products_to_update), batch_size):
#                     batch = products_to_update[i:i + batch_size]
#                     Product.objects.bulk_update(batch, attrs_updata)
#
#     def delete_file(self,file_instance):
#         from django.core.files.storage import default_storage
#         try:
